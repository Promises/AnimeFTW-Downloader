		...
		if self.settings['token'] == '':
			...
			if self.settings['username'] == '' or self.settings['password'] == '':
				...
				if 'status' in response and response['status'] == "200":
					...
					self.settings['token'] = value=response['message']
					#SETTINGS.setSetting('hennber', value="")
...
class grabFTW:
	...
	def getEpisodes(self, url, seriesid = None, seriesimage = None, category = None):
		data = {"id":seriesid,"action":"display-episodes","start":"0","count":"200"} #episode strt nd end :DDDD
		action = "Display Category Listing."
		jsonSource = self.getContent(data, action)
		parsed_json = json.loads(jsonSource)
		#print parsed_json
		...
		if 'status' in parsed_json and parsed_json['status'] == "200":
			...
			for episode in episode_results:
				...
				#print episode
				if category == 'episode' or category == 'fullmetal' or category == 'fullmetal2':
				...
				#print epname + ', ' +  url + ', '  + thumbnail
				#print url
				print epname



			print "Welcome to hennings anime downloader"
			coice = input("Please select episode number: ")
			coice = int(coice)
			self.getEpisodesForDown(coice, seriesid, seriesimage, category)

	def getEpisodesForDown(self, epid, seriesid = None, seriesimage = None, category = None):
		data = {"id":seriesid,"action":"display-episodes","start":epid,"count":"1"} #episode strt nd end :DDDD
		action = "Display Category Listing."
		jsonSource = self.getContent(data, action)
		parsed_json = json.loads(jsonSource)
		#print parsed_json
		...
		if 'status' in parsed_json and parsed_json['status'] == "200":
			...
			for episode in episode_results:
				...
				if thumbnail == "http://img02.animeftw.tv/video-images/noimage.png":
					...
					#print epname + ', ' +  url + ', '  + thumbnail
					#print url
				self.download_file(url, epname, "TODO Seriesname")
...
'''
	def download_file(self, url, name, seiersname):
		print "Downloading"
		stream_url = url.replace(' ', '')
		stream_url += '?Referrer=www.animeftw.tv'
		headers = {'User-Agent': 'Kodi/15.2 (Macintosh; Intel Mac OS X 10_11_1) App_Bitness/64 Version/15.2-Git:2015-10-19-02e7013'}
		local_filename = seiersname + "/" + name + '.mp4'
		# NOTE the stream=True parameter
		if not os.path.exists(seiersname):
			os.makedirs(seiersname)
...
class UI:
	...
	def showCategories(self):
		...
		self.addItem({'Title':'Full Metal Alchemist', 'mode':'fullmetal'})
		self.addItem({'Title':'Full Metal Alchemist: Brotherhood', 'mode':'fullmetal2'})
		self.endofdirectory()
...
'''
	def mainui(self):
		...
		grabFTW().getEpisodes('None', "160", 'None', 'fullmetal2')
	def watchlist(self):
		grabFTW().getEpisodes(self.main.args.url, "3", self.main.args.icon, self.main.args.mode)
...
class Main:
	...
	def parseArgs(self):
		...
		self.args = updateArgs(mode = 'series', url = 'None', name = 'None')

	def checkMode(self):
		mode = None
		print "[FTW] Current mode is: "
		if mode is None:
			UI().mainui()
		elif mode == 'series':
			UI().showAnimeSeries()
